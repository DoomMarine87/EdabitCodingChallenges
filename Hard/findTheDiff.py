"""You are given two strings s and t. String t is generated by randomly shuffling string s and then adding one more letter at a random 
position. Return the letter that was added to t.
Examples

find_the_difference("abcd", "abcde") ➞ "e"

find_the_difference("", "y") ➞ "y"

find_the_difference("ae", "aea") ➞ "a"""

def find_the_difference(s, t):
    return"".join(i for i in t if t.count(i) != s.count(i))[0]  
        

print(find_the_difference("abcd", "abcde"))# ➞ "e"
print(find_the_difference("", "y"))# ➞ "y"
print(find_the_difference("ae", "aea"))# ➞ "a""

"""def find_the_difference(s, t):
	return chr(sum(map(ord, t))-sum(map(ord, s)))"""

"""def find_the_difference(s, t):
	for i in s:
		t = t.replace(i,'',1)
	return t"""

from time import perf_counter
tic = perf_counter()

print('t = {:.6f}'.format(perf_counter() - tic))